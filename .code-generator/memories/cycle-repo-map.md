...
## Corpus Check
- 53 files · ~84,886 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1931 nodes · 2767 edges · 56 communities detected
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 550 edges (avg confidence: 0.75)
...
1. `BaseEstimator` - 89 edges
2. `EncoderDecoderForecaster` - 73 edges
3. `DMMForecaster` - 42 edges
4. `TimeSeriesDataset` - 42 edges
...
Cohesion: 0.01
Nodes (166): Unpack data returned by the net's iterator into a 2-tuple.      This function is, unpack_data(), BaseEstimator, _current_init_context(), _extract_optimizer_param_name_and_group(), history(), optimizer_setter(), predict() (+158 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (91): Prepare the scoring callback at the beginning of training.          This method, Acquire the data required for evaluating the scoring function.          This met, Dataset, Enum, NotFittedError, Custom dataset class for Stockpy designed to handle a variety of data types., Initializes the `StockpyDataset`., Get the length of the dataset.          Returns         -------         int (+83 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (51): Combiner, EmitterRegressor, Sub-modules used by ``DMMForecaster``.  Encapsulates the inference (``Combiner``, Gaussian latent-state transition p(z_t | z_{t-1}, x_t).      Implements a gated, Return ``(loc, scale)`` of p(z_t | z_{t-1}, x_t)., Parameterizes the variational distribution q(z_t | z_{t-1}, x_{t:T}).      Maps, Return ``(loc, scale)`` of q(z_t | z_{t-1}, x_{t:T})., Parameterizes the Gaussian observation likelihood p(y_t | z_t, x_t).      Genera (+43 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (40): _build_decoder(), _build_encoder(), _build_table(), _PositionalEncoding, Transformer encoder-decoder forecaster.  Implements feature 11 of the v0.4.0 res, Encode ``x`` then decode ``pred_len`` future steps.          Parameters, Transformer encoder-decoder forecaster.      Parameters     ----------     d_mod, Build :class:`TransformerModel` from ``self.n_features_in_``. (+32 more)
...
Cohesion: 0.03
Nodes (47): Calculate the weighted average of scores for the latest epoch.          This met, list, Retrieve and return initialization parameters for a specific optimizer., Retrieve and return initialization parameters for a specific optimizer., Retrieve initialization parameters for a specified optimizer.          This publ, Retrieve initialization parameters for a specified optimizer.          This publ, from_file(), _get_getitem_method() (+39 more)
...
Cohesion: 0.05
Nodes (29): DMMForecaster, Deep Markov Model encoder-decoder forecaster.      Parameters     ----------, Deep Markov Model encoder-decoder forecaster.      Parameters     ----------, Deep Markov Model encoder-decoder forecaster.      Parameters     ----------, Override to create a Pyro optimizer for SVI., Generative model for SVI., Variational guide for SVI., Forward pass through the DMM model.          Parameters         ---------- (+21 more)
...
Cohesion: 0.06
Nodes (21): _compute_num_patches(), _PatchEmbedding, PatchTSTForecaster, _PatchTSTModel, PatchTST encoder-only forecaster with channel-independent patching.  Reference:, Project patches and add positional encoding.          Parameters         -------, Top-level module: RevIN → patch → TransformerEncoder → head.      Parameters, Encode ``x`` and forecast ``pred_len`` steps.          Parameters         ------ (+13 more)
...
Cohesion: 0.05
Nodes (31): EpochTimer, filter_log_keys(), PrintLog, Callbacks for printing, logging and log information., Initializes the callback before training begins.          This method sets up th, Initializes the callback before training begins.          This method sets up th, Yield keys from an iterable that are not designated to be ignored for logging., Yield keys from an iterable that are not designated to be ignored for logging. (+23 more)

### Community 18 - "Community 18"
Cohesion: 0.06
Nodes (17): _iTransformerEmbedding, iTransformerForecaster, _iTransformerModel, iTransformer encoder-only forecaster with variates-as-tokens.  Reference: Liu et, Encode ``x`` and forecast ``pred_len`` steps.          Parameters         ------, iTransformer encoder-only forecaster.      Parameters     ----------     d_model, Per-variate linear projection L -> d_model.      Shared weights across all C var, Build :class:`_iTransformerModel` from stored hyperparameters. (+9 more)
...
Cohesion: 0.12
Nodes (17): Exception, Check if the attribute is settable in the current context.          Ensures that, Check if the attribute is settable in the current context.          Ensures that, DataDownloader, _fetch_sp500_symbols(), main(), _normalize_symbol(), Normalize a ticker symbol to Yahoo Finance form.      Yahoo Finance uses ``-`` i (+9 more)
...
Cohesion: 0.33
Nodes (4): _linkcode_resolve(), project_linkcode_resolve(), Determine a link to online source for a class/method/function      This is calle, Determine a link to online source for a class/method/function      This is calle
...
Cohesion: 1.0
Nodes (1): Compute number of patches after end-padding.
...
## Knowledge Gaps
- **761 isolated node(s):** `Normalize a ticker symbol to Yahoo Finance form.      Yahoo Finance uses ``-`` i`, `Scrape S&P 500 ticker list from Wikipedia.      Wikipedia rejects the default ur`, `Tests for safetensors save/load round-trip on encoder-decoder forecasters.  Cove`, `Round-trip save → load → predict via safetensors with no workarounds.`, `Packaging tests for issue #37.  Asserts that ``setup.py`` is gone, ``pyproject.t` (+756 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 38`** (2 nodes): `TestDifferenceTransform`, `TestStandardScalerTransform`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (2 nodes): `stockpy.base module`, `stockpy package init`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (2 nodes): `DifferenceTransform`, `StandardScalerTransform`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `Default callbacks used during training and validation processes.          This p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (1 nodes): `A context manager to temporarily set the current initialization context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (1 nodes): `Perform a validation step, compute and return the loss, and possibly predictions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (1 nodes): `Forecast future time steps for the given input sequences.          Subclasses mu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (1 nodes): `Create a History instance from a JSON file.          Parameters         --------`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (1 nodes): `Retrieves keyword arguments for the scheduler.          This property filters ou`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (1 nodes): `Constructs the full path for the training history file.          This property a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (1 nodes): `Compute number of patches after end-padding.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (1 nodes): `Default callbacks used during training and validation processes.          This p`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (1 nodes): `A context manager to temporarily set the current initialization context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (1 nodes): `Perform a validation step, compute and return the loss, and possibly predictions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (1 nodes): `Forecast future time steps for the given input sequences.          Subclasses mu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (1 nodes): `Forecast future time steps for the given input sequences.          Subclasses mu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (1 nodes): `setup() entrypoint`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (1 nodes): `get_activation_function`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (1 nodes): `ValidSplit`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `EncoderDecoderForecaster` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 6`, `Community 7`, `Community 8`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 18`, `Community 19`, `Community 20`, `Community 26`?**
  _High betweenness centrality (0.399) - this node is a cross-community bridge._
- **Why does `BaseEstimator` connect `Community 0` to `Community 1`, `Community 3`, `Community 6`, `Community 26`, `Community 28`?**
  _High betweenness centrality (0.228) - this node is a cross-community bridge._
- **Why does `History` connect `Community 6` to `Community 0`, `Community 3`, `Community 9`, `Community 17`, `Community 21`?**
...
- **Are the 65 inferred relationships involving `EncoderDecoderForecaster` (e.g. with `BaseEstimator` and `TestiTransformerForecasterContract`) actually correct?**
  _`EncoderDecoderForecaster` has 65 INFERRED edges - model-reasoned connections that need verification._
