...
class CNNRegressor(EncoderDecoderForecaster, CNN):
    """
    CNNRegressor is a convolutional neural network designed for forecasting tasks.

    It inherits from the EncoderDecoderForecaster and CNN classes. This class is particularly suited to forecasting problems where
    the input data have a grid-like topology, such as time-series data or image pixels.

    Parameters
    ----------
    Inherits all parameters from the `EncoderDecoderForecaster` and `CNN` classes.
