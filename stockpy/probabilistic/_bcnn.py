...
class BayesianCNNClassifier(ClassifierProb):
    ...
    _init_model(self):
        Initializes the Convolutional layers and fully connected layers of the model 
    ...
    forward(self, x_data: torch.Tensor, y_data: torch.Tensor=None) -> torch.Tensor:
        Defines the forward pass of the Bayesian CNN, and optionally observes 
        the output if ground truth `y_data` is provided.

    _initSVI(self) -> pyro.infer.svi.SVI:
        Initializes Stochastic Variational Inference (SVI) for the model.
        Defines the guide function to be a Normal distribution that learns 
        to approximate the posterior, and uses Mean Field ELBO as the 
        variational loss.
    """
    ...
    def _init_model(self):
        ...
        # Creates the FC layers of the neural network with Pyro's Module wrapper
        ...
        # Adds the output FC layer with Pyro's Module wrapper
        layers.append(PyroModule[nn.Linear](current_input_size, self.output_size))

        # Creates the neural network as a sequential model based on the layers list with Pyro's Module wrapper
