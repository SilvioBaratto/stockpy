...
__all__ = ['PassthroughScoring', 'EpochScoring', 'ScoringBase']
...
def cache_net_infer(net, use_caching, y_preds):
    ...
    Examples:
        >>> with cache_net_infer(net, use_caching=True, y_preds=cached_preds) as cached_net:
        ...     # Use cached_net for operations that require predictions
        ...     pass
        >>> # Outside the context, net's infer method is back to its original state
    """
...
def _cache_net_forward_iter(net, use_caching, y_preds):
    """
    Context manager for caching the output of forward passes in skorch.NeuralNet.
    ...
    Examples:
        >>> with _cache_net_forward_iter(net, use_caching=True, y_preds=cached_outputs) as cached_net:
        ...     # Use cached_net for operations that require forward passes
        ...     pass
        >>> # Outside the context, net's forward_iter method is back to its original state

    Notes:
        - The caching behavior can be controlled by the `use_caching` parameter of the `skorch.NeuralNet`
...
class ScoringBase(Callback):
    ...
    def _get_name(self):
        ...
        Examples:
            ...
            >>> scoring = partial(accuracy_score, normalize=False)
            >>> callback = ScoringBase(scoring=scoring)
            >>> callback._get_name()
            'accuracy_score'
    ...
    def initialize(self):
        ...
        Examples:
            >>> from sklearn.metrics import accuracy_score
            >>> scoring_callback = ScoringBase(scoring=accuracy_score)
            >>> scoring_callback.initialize()
            <__main__.ScoringBase object at 0x...>
    ...
    def on_train_begin(self, net, X, y, **kwargs):
        ...
        Examples:
            >>> # Assuming `net` is an instance of `skorch.NeuralNet`
            >>> scoring_callback = ScoringBase(scoring=accuracy_score)
            >>> scoring_callback.on_train_begin(net, X_train, y_train)
    ...
    def _scoring(self, net, X_test, y_test):
        ...
        # Check and get the scoring function compatible with the Skorch framework
...
class EpochScoring(ScoringBase):
    ...
    def get_test_data(self, dataset_train, dataset_valid, use_caching):
        ...
        Parameters:
            dataset_train : skorch.Dataset or similar
                The training dataset or an object that can be used to generate
                training data batches. This is not necessarily a skorch Dataset
                but must be compatible with skorch's expectations of a dataset.
            
            dataset_valid : skorch.Dataset or similar
...
class PassthroughScoring(Callback):
    ...
    def initialize(self):
        ...
        Examples:
            >>> scoring = PassthroughScoring(name='accuracy')
            >>> scoring.initialize()
            <PassthroughScoring>
        """
        self.best_score_ = np.inf if self.lower_is_better else -np.inf
