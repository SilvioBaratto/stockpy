...
class LRScheduler(Callback):
    ...
    Parameters:
        policy : str or _LRScheduler class, optional (default='WarmRestartLR')
        ...
        monitor : str or callable, optional (default='train_loss')
            ...
            that returns a score (float) when passed the net instance.

        event_name : str, optional (default='event_lr')
            ...
            This feature is only supported in PyTorch version 1.4 or later.

        step_every : str, optional (default='epoch')
            Determines when the scheduler should take a step. Can be 'epoch' to step
            after each epoch or 'batch' to step after each training batch.
    ...
    def simulate(self, steps, initial_lr):
        """
        Simulates the learning rate schedule over a specified number of steps.
    ...
    def on_train_begin(self, net, **kwargs):
        ...
        Examples:
            >>> # Assuming `net` is an instance of skorch.NeuralNet and training
            >>> # has not yet begun:
            >>> scheduler_cb = LRScheduler(policy='StepLR', step_size=5)
            >>> scheduler_cb.on_train_begin(net)
            # The scheduler is now initialized and ready for training.
    ...
    def _step(self, net, lr_scheduler, score=None):
        ...
        Examples:
            >>> # Assuming `net` is an instance of skorch.NeuralNet, `scheduler` is an
            >>> # instance of a learning rate scheduler, and `current_score` is the
            >>> # performance metric of interest:
            >>> self._step(net, scheduler, score=current_score)
            # The scheduler is stepped, adjusting the learning rate as needed.
...
class WarmRestartLR(_LRScheduler):
    ...
    def get_lr(self):
        """
        Computes the learning rate for each parameter group using the SGDR policy.

        This method considers the progress through the current period and adjusts
        the learning rate accordingly for each parameter group. The adjustment is
        based on the base period and its multiplication factor, accounting for
