...
class TestTCNForecaster:
    """Comprehensive tests for TCNForecaster encoder-decoder implementation."""

    def test_is_subclass_of_encoder_decoder_forecaster(self):
        assert issubclass(TCNForecaster, EncoderDecoderForecaster)

    def test_instantiation_sets_context_and_pred_len(self):
        model = TCNForecaster(context_len=10, pred_len=5)
        assert model.context_len == 10
        assert model.pred_len == 5

    def test_model_type_attribute(self):
        model = TCNForecaster(context_len=4, pred_len=2)
        assert model.model_type == "cnn"

    def test_initialize_module_creates_module_(self):
        model = TCNForecaster(context_len=4, pred_len=2, num_filters=8, hidden_size=8)
    ...
    def test_module_has_encoder_and_decoder(self):
        model = TCNForecaster(context_len=4, pred_len=2, num_filters=8, hidden_size=8)
        X = np.random.randn(20, 3).astype(np.float32)
        y = np.random.randn(20, 3).astype(np.float32)
        model.fit(X, y, epochs=0, verbose=0, train_split=None)
    ...
    def test_encoder_is_tcn(self):
        model = TCNForecaster(context_len=4, pred_len=2, num_filters=8, hidden_size=8)
        X = np.random.randn(20, 3).astype(np.float32)
        y = np.random.randn(20, 3).astype(np.float32)
        model.fit(X, y, epochs=0, verbose=0, train_split=None)
        # TCN encoder should contain causal convolutional blocks
        assert any("conv" in name.lower() for name, _ in model.module_.encoder.named_modules())

    def test_decoder_is_gru(self):
        model = TCNForecaster(context_len=4, pred_len=2, num_filters=8, hidden_size=8)
        X = np.random.randn(20, 3).astype(np.float32)
        y = np.random.randn(20, 3).astype(np.float32)
        model.fit(X, y, epochs=0, verbose=0, train_split=None)
        assert isinstance(model.module_.decoder, nn.GRU)
    ...
    def test_predict_returns_correct_shape(self):
        model = TCNForecaster(
            context_len=10, pred_len=5, num_filters=16, hidden_size=16, num_layers=1
        )
    ...
    def test_fit_on_synthetic_data(self):
        ...
        y = series
        model = TCNForecaster(
            context_len=20, pred_len=5, num_filters=16, hidden_size=16, num_layers=1
        )
        model.fit(X, y, epochs=2, verbose=0, train_split=None)
    ...
    def test_dropout_applies_in_training(self):
        model = TCNForecaster(
            context_len=4, pred_len=2, num_filters=8, hidden_size=8, dropout=0.5
        )
        X = np.random.randn(20, 3).astype(np.float32)
        y = np.random.randn(20, 3).astype(np.float32)
        model.fit(X, y, epochs=0, verbose=0, train_split=None)
    ...
    def test_save_load_roundtrip_safetensors(self, tmp_path):
        model = TCNForecaster(
            context_len=10, pred_len=5, num_filters=8, hidden_size=8, num_layers=1
        )
        X = np.random.randn(100, 3).astype(np.float32)
