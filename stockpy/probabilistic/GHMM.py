...
class GHMM():
    ...
    def __get_most_probable_outcome(self, X_test, day_index):
        ...
        previous_data_features = GHMM.__extract_features(previous_data)
    ...
    def __extract_features(dataframe):
        ...
        """
        open_price = np.array(dataframe['Open'], dtype=np.float32)
        close_price = np.array(dataframe['Close'], dtype=np.float32)
        high_price = np.array(dataframe['High'], dtype=np.float32)
        low_price = np.array(dataframe['Low'], dtype=np.float32)
