...
class TestLSTMForecaster:
    """Comprehensive tests for LSTMForecaster encoder-decoder implementation."""

    def test_is_subclass_of_encoder_decoder_forecaster(self):
        assert issubclass(LSTMForecaster, EncoderDecoderForecaster)

    def test_instantiation_sets_context_and_pred_len(self):
        model = LSTMForecaster(context_len=10, pred_len=5)
        assert model.context_len == 10
        assert model.pred_len == 5

    def test_model_type_attribute(self):
        model = LSTMForecaster(context_len=4, pred_len=2)
        assert model.model_type == "rnn"

    def test_initialize_module_creates_module_(self):
        model = LSTMForecaster(context_len=4, pred_len=2, rnn_size=8, hidden_size=8)
