...
class BaseProb(BaseEstimator, PyroModule, metaclass=CombinedMeta):
    ...
    def _initOptimizer(self) -> torch.optim.Optimizer:
        ...
        torch.optim.Optimizer: 
            The optimizer instance used to train the model.
        """
    ...
    def _initScheduler(self) -> torch.optim.lr_scheduler.StepLR:
        ...
        torch.optim.lr_scheduler.StepLR: 
            The learning rate scheduler used to control the learning rate during training.
        """
    ...
    def _initSVI(self) -> pyro.infer.svi.SVI:
        ...
        -------
        pyro.infer.svi.SVI: 
            The SVI instance used to optimize the model and guide.
        """
    ...
    def _log_build_file_path(self):
        ...
        -------
        dict: 
            The configuration dictionary for the file path for logging.
        """
    ...
    def _log_model_state(self):
        ...
        -------
        dict: 
            The dictionary containing the state of the model for logging.
        """
...
class ClassifierProb(BaseProb, ClassifierMixin, metaclass=ABCMeta):
    ...
    def _doTraining(self,
        ...
        """
        # Initializes the total training loss, total correct predictions, total data points, and lists to store true and predicted labels
        train_loss = 0.0
        correct = 0
        total = 0
        true_labels = []
        ...
        # Switches the model to training mode. This has any effect only on certain modules like Dropout or BatchNorm.
        self.train()
        ...
        for x_batch, y_batch in train_dl:
            ...
            # The loss is added to the total training loss
            train_loss += loss
         
            # The model makes a prediction using the current batch of data
            output = self.forward(x_batch)
            # Gets the predicted class by finding the maximum value of the output
            _, predicted = torch.max(output.data, 1)
            # Adds the number of data points in the batch to the total number of data points
            total += y_batch.size(0)
            # Adds the number of correct predictions in the batch to the total number of correct predictions
            correct += (predicted == y_batch).sum().item()

            # Extends the list of true labels and predicted labels
            true_labels.extend(y_batch.tolist())
            pred_labels.extend(predicted.tolist())

        # Computes the average training loss
        train_loss /= len(train_dl)
        # Computes the weighted f1-score, a measure of the model's performance
        train_f1 = f1_score(true_labels, pred_labels, average='weighted') * 100

        # Returns the training loss, f1-score, true labels, and predicted labels
...
class RegressorProb(BaseProb, RegressorMixin, metaclass=ABCMeta):
    ...
    def _predictNN(self,
        ...
        ----------
        test_dl : torch.utils.data.DataLoader
            DataLoader object that contains the test data in batches.
