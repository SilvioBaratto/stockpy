...
__all__ = ["BiGRUForecaster"]


class BiGRUModel(nn.Module):
    """BiGRU-based encoder-decoder for time-series forecasting.

    The encoder is a bidirectional GRU that processes the context window.
    The decoder is a unidirectional GRU that autoregressively produces the
    prediction horizon, with optional teacher-forcing during training.
    ...
    rnn_size : int, default=32
        Hidden size of each GRU direction in the encoder.
    ...
    def __init__(
        ...
        # Decoder hidden size must match the concatenated bidirectional output
        decoder_hidden_size = 2 * rnn_size
        self.decoder = nn.GRU(
            input_size=n_features,
        ...
        self.output_proj = nn.Sequential(
            nn.Linear(decoder_hidden_size, hidden_size, bias=bias),
            act,
            nn.Dropout(dropout),
            nn.Linear(hidden_size, n_features, bias=bias),
        )

    def _init_decoder_state(self, encoder_hidden: torch.Tensor) -> torch.Tensor:
        """Reshape encoder hidden for the unidirectional decoder.

        PyTorch bidirectional GRU returns hidden of shape
        ``(num_layers * 2, batch, rnn_size)``: forward layers first,
        then backward layers.  We pair and concatenate the forward and
        backward vectors for each layer so the decoder receives
        ``(num_layers, batch, 2 * rnn_size)``.
        """
        # encoder_hidden: (num_layers * 2, batch, rnn_size)
        batch_size = encoder_hidden.size(1)
        # Reshape to (num_layers, 2, batch, rnn_size)
        h = encoder_hidden.view(self.num_layers, 2, batch_size, self.rnn_size)
        # Concatenate forward (idx 0) and backward (idx 1) along feature dim
        h = torch.cat([h[:, 0], h[:, 1]], dim=-1)  # (num_layers, batch, 2*rnn_size)
        return h
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if y is not None:
            ...
            out, _ = self.decoder(decoder_input, dec_h)
            # out: (batch, pred_len, 2 * rnn_size)
            ...
            for _ in range(self.pred_len):
                out, dec_h = self.decoder(decoder_input, dec_h)
                # out: (batch, 1, 2 * rnn_size)
...
class BiGRUForecaster(EncoderDecoderForecaster):
    """BiGRU-based encoder-decoder forecaster.

    Parameters
    ----------
    rnn_size : int, default=32
        Number of features in each GRU hidden state direction.
    hidden_size : int, default=32
    ...
    **kwargs
    ...
    def initialize_module(self) -> BiGRUForecaster:
        """Create the BiGRU encoder-decoder module and loss criterion."""
        self.module_ = BiGRUModel(
    ...
    def forward(  # type: ignore[override]
        ...
        """Forward pass through the BiGRU model.
